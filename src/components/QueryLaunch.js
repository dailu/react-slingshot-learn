...
class QueryLaunch extends Component {
  
  queryRequest=null;
  queryRequestCancel=null;
  ...
  onQuery = async (e, dropCommandIsConfirmed = false) => {
    ...
    try {
      ...
      this.queryRequest = await axios.post(databaseEndpoint(true)+"&add_http_cors_header=1", `${query} FORMAT JSON`, {
  ...
  render() {
    return (
      <div id="editor" style={{ height: '100%' }}>
        ...
        />
        <KeyboardHelp
          ref={instance => { this.keyboardHelp = instance; }}
        />
        <Alert
        ...
        >
          ...
          <NavbarGroup align={Alignment.RIGHT} style={{ height: '35px' }}>
            ...
            <Tooltip content="Querys History Dialog" position={Position.LEFT}>
              <Button
...
QueryLaunch.propTypes = {
  onData: object
};


export default QueryLaunch;